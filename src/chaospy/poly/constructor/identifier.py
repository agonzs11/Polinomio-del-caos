...
def identify_core(core):
    ...
    for datatype, identifier in {
            ...
            dict: _identify_dict,
            chaospy.poly.fraction.frac: _identify_frac,
...
def _identify_frac(core):
    """Specification for a fraction."""
    return {(0,): core}, 1, core.shape, chaospy.poly.fraction.frac
        ...
        for key in core_.keys:
            ...
            if not key in out:
                if dtype == chaospy.poly.fraction.frac:
                    out[key] = chaospy.poly.fraction.frac(np.zeros(shape))
                else:
