...
def diag(A, k=0):
    ...
    elif isinstance(A, fraction.frac):
        return fraction.frac(
            np.diag(A.a, k),
            np.diag(A.b, k),
        )

    elif isinstance(A, Poly):
        core, core_new = A.A, {}
        for key in A.keys:
...
def repeat(A, repeats, axis=None):
    ...
    if isinstance(A, (int, float, np.ndarray)):
        ...
        return fraction.frac(
            np.repeat(A.a, repeats, axis),
            np.repeat(A.b, repeats, axis),
        )
...
def trace(A, offset=0, ax1=0, ax2=1):
    ...
    if isinstance(A, (int, float, list, tuple, np.ndarray)):
        ...
        return frac(
            np.trace(A.a, offset, ax1, ax2),
            np.trace(A.b, offset, ax1, ax2),
        )
