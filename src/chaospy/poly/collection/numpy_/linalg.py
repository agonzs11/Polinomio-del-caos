...
def inner(*args):
    ...
    hasfrac = sum([isinstance(arg, chaospy.poly.fraction.frac)
                   for arg in args])
    ...
    # Frac
    if not haspoly:
        if not np.any([arg.shape for arg in args]):
            denom = [arg.a for arg in args]
            nume = [arg.b for arg in args]
            return chaospy.poly.collection.prod(
                chaospy.poly.fraction.frac(denom, nume))

        denom = np.array([_.a for _ in args])
        denom = np.prod(denom, 0)
        nume = np.array([_.b for _ in args])
        nume = np.prod(nume, 0)
        return sum(chaospy.poly.fraction.frac(denom, nume), 0)
...
def outer(*args):
    ...
    if dtype == chaospy.poly.fraction.frac:
        ...
        return chaospy.poly.fraction.frac(
            outer(part1.a, part2.a), # pylint: disable=no-member
            outer(part2.a, part2.a), # pylint: disable=no-member
        )
