...
class Poly(object):
    ...
    def __div__(self, other):
        """Python2 division."""
        if isinstance(y, (float, int, chaospy.poly.fraction.frac)):
            return chaospy.poly.collection.arithmetics.mul(self, other**-1)
    ...
    def __str__(self):
        ...
        if self.shape:
            ...
            if len(self.shape) > 1:
                shape = self.shape
                self = poly.shaping.flatten(self)
        ...
        for key in self.keys[::-1]:
            ...
            o = ""
            coef = self.A[key]
            if isinstance(coef, chaospy.poly.fraction.frac):
